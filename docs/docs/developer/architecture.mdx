...
The  Photosync  CLI is an [npm](https://www.npmjs.com/) package that lets users control their  Photosync  instance from the command line. It uses the API to perform various tasks, especially uploading assets. See the [CLI documentation](/docs/features/command-line-interface.md) for more information.

## Server

The  Photosync  backend is divided into several services, which are run as individual docker containers.
...
The  Photosync  Microservices image uses the same `Dockerfile` as the  Photosync  Server, but with a different entrypoint. The  Photosync  Microservices service mainly handles executing jobs, which include the following:
