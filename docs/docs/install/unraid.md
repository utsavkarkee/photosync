...
In order to install  Photosync  from the Unraid CA, you will need an existing Redis and PostgreSQL 14 container, If you do not already have Redis or PostgreSQL you can install them from the Unraid CA, just make sure you choose PostgreSQL **14**.

Once you have Redis and PostgreSQL running, search for  Photosync  on the Unraid CA, choose either of the templates listed and fill out the example variables.
...
7.  Paste the entire contents of the [Immich example.env](https://github.com/wonderkidshihab/photo-backup/releases/latest/download/example.env) file into the Unraid editor, then **before saving** edit the following:
    ...
    - `DB_DATA_LOCATION`: Change this to use an Unraid share (preferably a cache pool, e.g. `/mnt/user/appdata`). If left at default it will try to use Unraid's `/boot/config/plugins/compose.manager/projects/[stack_name]/postgres` folder which it doesn't have permissions to, resulting in this container continuously restarting.
      ...
      width="70%"
      alt="Absolute location of where you want  Photosync  images stored"
      />
...
8.  Click "**Save Changes**" followed by "**Compose Up**" and Unraid will begin to create the  Photosync  containers in a popup window. Once complete you will see a message on the popup window stating _"Connection Closed"_. Click "**Done**" and go to the Unraid "**Docker**" page

    > Note: This can take several minutes depending on your Internet speed and Unraid hardware

9.  Once on the Docker page you will see several  Photosync  containers, one of them will be labelled `immich_server` and will have a port mapping. Visit the `IP:PORT` displayed in your web browser and you should see the  Photosync  admin setup page.
...
You should ignore the "_update ready_" on the Unraid WebUI and update when you receive the notification within the  Photosync  WebUI.
<img
...
2. Next to  Photosync  click the "**Update Stack**" button and Unraid will begin to update all  Photosync  related containers
...
3. Once complete you will see a "_Connection Closed_" message, select "**Done**".
   ...
   />
4. Return back to the  Photosync  WebUI and you will see the version has been updated to the latest
   <img
