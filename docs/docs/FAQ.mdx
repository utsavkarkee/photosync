...
On iOS (iPhone and iPad), the operating system determines if a particular app can invoke background tasks based on multiple factors, most of which the  Photosync  app has no control over. To increase the likelihood that the background backup task is run, follow the steps below:

- Enable Background App Refresh for  Photosync  in the iOS settings at `Settings > General > Background App Refresh`.
...
- Use the  Photosync  app more often.
...
### Does  Photosync  change the file?

No,  Photosync  does not modify the original files.
All edited metadata is saved in companion `.xmp` sidecar files and the database.
However,  Photosync  will delete original files that have been trashed when the trash is emptied in the  Photosync  UI.

### Why do my file names appear as a random string in the file manager?

When Storage Template is off (default)  Photosync  saves the file names in a random string (also known as random UUIDs) to prevent duplicate file names. To retrieve the original file names, you must enable the Storage Template and then run the STORAGE TEMPLATE MIGRATION job.
...
### Does  Photosync  support reading existing face tag metadata?

No, it currently does not. There is an [open feature request on GitHub](https://github.com/wonderkidshihab/photo-backup/discussions/4348).

### Does  Photosync  support the filtering of NSFW images?

No, it currently does not. There is an [open feature request on Github](https://github.com/wonderkidshihab/photo-backup/discussions/2451).
...
Files sent on WhatsApp are saved without metadata on the file. Therefore,  Photosync  has no way of knowing the original date of the file when files are uploaded from WhatsApp, not the order of arrival on the device. [See #3527](https://github.com/wonderkidshihab/photo-backup/issues/3527).
...
### Why do HDR videos appear pale in  Photosync  player but look normal after download?
...
### Why does  Photosync  transcode my videos to a lower quality?
...
No, not yet. For updates on this planned feature, follow the [GitHub discussion](https://github.com/wonderkidshihab/photo-backup/discussions/1689).
...
We haven't implemented an official mechanism for creating albums from external libraries, but there are some [workarounds from the community](https://github.com/wonderkidshihab/photo-backup/discussions/4279) to help you achieve that.
...
In read-only libraries (`:ro` in the `docker-compose.yml`),  Photosync  is unable to create the `.xmp` sidecar files to store edited file metadata.
...
In read-write external libraries (the default),  Photosync  will delete the original file.
In read-only libraries (`:ro` in the `docker-compose.yml`), files can still be trashed in the UI.
However, when the trash is emptied, the files will re-appear in the main timeline since  Photosync  is unable to delete the original file.
...
### Does  Photosync  support Facial Recognition for videos?
...
### Does  Photosync  have animal recognition?
...
### Why is  Photosync  slow on low-memory systems like the Raspberry Pi?
...
### How can I run  Photosync  as a non-root user?
...
Data for  Photosync  comes in two forms:
...
```bash title="Remove  Photosync  (containers and volumes)"
docker compose down -v
```

After removing the containers and volumes, there are a few directories that need to be deleted to reset  Photosync  to a new installation. Once they are deleted,  Photosync  can be started back up and will be a fresh installation.
...
and remove all the volumes related to  Photosync  then restart the stack.
:::
...
If it mentions SIGILL (note the lack of a K) or error code 132, it most likely means your server's CPU is incompatible. This is unlikely to occur on version 1.92.0 or later. Consider upgrading if your version of  Photosync  is below that.

If your version of  Photosync  is below 1.92.0 and the crash occurs after logs about tracing or exporting a model, consider either upgrading or disabling the Tag Objects job.
...
If you installed  Photosync  using v1.104.0 or later, you likely have database checksums enabled by default. You can check this by running the following command.
